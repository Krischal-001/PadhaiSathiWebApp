...
export default function TutorPublicProfile() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [t, r] = await Promise.all([getTutorById(id), getTutorReviews(id)]);
    ...
    <div style={{ display: "flex", gap: 2 }}>
      {[1,2,3,4,5].map(n => (
        <span key={n}
          ...
          style={{ fontSize: interactive ? 28 : 15, color: n <= rating ? "#f59e0b" : "#d1d5db", cursor: interactive ? "pointer" : "default" }}>
          &#9733;
        </span>
      ))}
    </div>
  ...
  if (!tutor) return <div style={{ padding: 60, textAlign: "center", color: "#6b7280", fontFamily: "system-ui" }}>Tutor not found.</div>;
      ...
      <div style={{ maxWidth: 860, margin: "0 auto", padding: "32px 24px" }}>
          ...
          <div style={{ display: "flex", gap: 24, alignItems: "flex-start", flexWrap: "wrap" }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 6 }}>
                <h1 style={{ fontSize: 26, fontWeight: 800, color: "#111", margin: 0 }}>{tutor.full_name || tutor.username}</h1>
                {tutor.is_verified && <span style={{ padding: "2px 10px", background: "#d1fae5", color: "#065f46", borderRadius: 20, fontSize: 12, fontWeight: 700 }}>Verified</span>}
              </div>
              <div style={{ display: "flex", gap: 16, flexWrap: "wrap", marginBottom: 12, fontSize: 14, color: "#6b7280" }}>
                ...
                {reviewData.total > 0 && (
                  <span style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    <Stars rating={Math.round(reviewData.avg_rating)} />
                    <span style={{ fontWeight: 600, color: "#111" }}>{reviewData.avg_rating}</span>
                    <span>({reviewData.total} reviews)</span>
                  </span>
                )}
              </div>
              <p style={{ fontSize: 15, color: "#374151", lineHeight: 1.7, margin: "0 0 16px" }}>{tutor.bio}</p>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                {tutor.subjects?.map(s => (
                  <span key={s.id} style={{ padding: "4px 12px", background: "#ede9fe", color: "#4f46e5", borderRadius: 20, fontSize: 13, fontWeight: 600 }}>{s.name}</span>
                ))}
              </div>
            </div>
        ...
        <div style={{ background: "#fff", border: "1px solid #e5e7eb", borderRadius: 16, padding: "28px 32px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
            <div>
              <h2 style={{ fontSize: 20, fontWeight: 700, color: "#111", margin: "0 0 4px" }}>Reviews</h2>
              {reviewData.total > 0 && (
          ...
          {showForm && (
            <form onSubmit={handleReview} style={{ background: "#f9fafb", borderRadius: 12, padding: 20, marginBottom: 24, border: "1px solid #e5e7eb" }}>
              ...
              <div style={{ marginBottom: 14 }}>
                <label style={{ fontSize: 13, fontWeight: 600, color: "#374151", display: "block", marginBottom: 6 }}>Booking ID (completed sessions only)</label>
                <input type="number" value={form.booking_id} onChange={e => setForm({ ...form, booking_id: e.target.value })}
                  placeholder="Your booking ID" required
                ...
                <div key={r.id} style={{ padding: "16px 0", borderBottom: "1px solid #f3f4f6" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                    <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                      <div style={{ width: 36, height: 36, borderRadius: "50%", background: "#ede9fe", color: "#4f46e5", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: 14 }}>
                        {(r.full_name || r.username)?.[0]?.toUpperCase()}
                      </div>
                      <div>
                        <p style={{ fontWeight: 600, fontSize: 14, color: "#111", margin: 0 }}>{r.full_name || r.username}</p>
