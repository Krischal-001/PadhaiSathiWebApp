        ...
        <div style={{ display: "flex", gap: 2, flex: 1 }}>
          {navLinks.map((l) => (
            <button key={l.path} onClick={() => navigate(l.path)} style={{
              background: "none", border: "none", padding: "6px 14px", cursor: "pointer", fontSize: 14, fontWeight: 500,
          ...
          {user ? (
            <>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ width: 36, height: 36, borderRadius: "50%", background: "linear-gradient(135deg, #4f46e5, #7c3aed)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 700, flexShrink: 0 }}>
                  {(user.full_name || user.username)?.[0]?.toUpperCase()}
                </div>
                <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                  <span style={{ fontSize: 13, fontWeight: 600, color: "#111", lineHeight: 1 }}>{user.full_name || user.username}</span>
                  <span style={{ fontSize: 10, fontWeight: 700, padding: "1px 7px", borderRadius: 10, textTransform: "capitalize", background: rc.bg, color: rc.color }}>
                    {user.role}
                  </span>
                </div>
              </div>
